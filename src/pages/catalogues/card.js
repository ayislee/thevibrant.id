  ...
  return (
    ...
    >
      ...
      >
        ...
        {displayText ? (
          <div>
            <ContentProduct style={{paddingBottom: (item.catalogue_product_name || item.catalogue_product_additional_json.text) ? '1rem' : 0}}>
              { item.catalogue_product_name && <Title>{item.catalogue_product_name}</Title> }
              { item.catalogue_product_additional_json.text && <div>{item.catalogue_product_additional_json.text}</div> }
            </ContentProduct>
            
            {catalogue_product_type ===
              "PROMO" ? null : item.catalogue_product_discount_price ? (
...
const Overlay = styled.div`
  ...
  h4 {
    transform: ${({ degree }) =>
    degree ? `rotate(${degree}deg)` : "rotate(-45deg)"};
    font-weight: bold;
    font-size: 60pt;
    color: #fff;
    opacity: 0.5;
