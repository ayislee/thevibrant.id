...
instance.interceptors.request.use(async (config) => {
  var token = Cookies.get("token") ? await Cookie.get({ name: "token" }) : "";
...
async function refreshToken() {
  const refreshToken = Cookies.get("refreshToken")
    ? await Cookie.get({ name: "refreshToken" })
    : "";
...
instance.setToken = (token) => {
  ...
  Cookie.set({ name: "token", data: token });
};
    ...
    if (!isRefreshing) {
      ...
      return refreshToken()
        .then((res) => {
          ...
          Cookie.set({ name: "refreshToken", data: refreshToken });
          instance.setToken(token);
